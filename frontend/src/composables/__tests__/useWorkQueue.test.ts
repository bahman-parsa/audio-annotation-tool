...
describe('useWorkQueue', () => {
  const { setItems, items, filteredItems, selectItem, selectedItem, searchQuery, statusFilter } =
    useWorkQueue()
  ...
  it('filteredItems filters by status', () => {
    ...
    const untouched = createItem({ status: 'UNTOUCHED' })
