...
const selectedType = ref<
  'number' | 'crud' | 'medical' | 'measurement' | 'entity' | 'spelled'
>('number');
...
function handleSave() {
  if (selectedType.value === 'number') {
    ...
    emit('save', {
      type: 'MEDICAL_TERM',
      attributes: {
        category: medicalCategory.value,
        note: medicalNote.value.trim(),
      },
    });
  } else if (selectedType.value === 'measurement') {
    if (measurementValue.value === null || isNaN(measurementValue.value))
      return;
    emit('save', {
      type: 'MEASUREMENT',
      attributes: {
  ...
  } else if (selectedType.value === 'spelled') {
    if (!resolvedWord.value.trim()) return;
    emit('save', {
      type: 'SPELLED_OUT',
      attributes: { resolvedWord: resolvedWord.value.trim() },
    });
  } else {
...
<template>
  <div class="fixed inset-0 z-50" @click.self="emit('close')">
    <div
      class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-xl border p-6 w-96"
    >
      ...
      <div class="space-y-4">
        ...
        <template v-if="selectedType === 'number'">
          ...
          <div>
            ...
            <input
              ...
              :type="rendering === 'digits' ? 'number' : 'text'"
              class="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              :placeholder="rendering === 'digits' ? 'e.g. 12' : 'e.g. zwoelf'"
            />
          </div>
        </template>
        ...
        <template v-if="selectedType === 'medical'">
          <div>
            ...
            <select
              ...
              class="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-violet-500"
            >
              <option
                v-for="cat in medicalCategories"
                :key="cat.value"
                :value="cat.value"
              >
                {{ cat.label }}
        ...
        <template v-if="selectedType === 'measurement'">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1"
              >Normalized Value (optional)</label
            >
            <input
              v-model.number="measurementNormalized"
              type="number"
              class="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
              placeholder="e.g. 1.5"
            />
        ...
        <template v-if="selectedType === 'spelled'">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1"
              >Resolved Word</label
            >
            <input
              ...
              class="w-full px-3 py-2 border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500"
              placeholder="e.g. Cefuroxim"
            />
          </div>
          <div class="text-sm text-yellow-700 bg-yellow-50 p-3 rounded-md">
            Enter the final word that the spelling resolves to.
          </div>
        </template>
